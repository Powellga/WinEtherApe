...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log_light = QGraphicsEllipseItem(0, 0, 10, 10)
        self.log_light.setBrush(QColor(255, 0, 0))
        self.visualizer.scene.addItem(self.log_light)

        self.log_text = QGraphicsTextItem("writing to log")
        self.log_text.setDefaultTextColor(QColor(255, 0, 0))
        self.log_text.setFont(QFont("Arial", 10))
        self.visualizer.scene.addItem(self.log_text)

        self.update_log_indicator_position()
        self.log_light.setVisible(False)
    ...
    def handle_new_packet(self, src_ip, dst_ip, protocol, size):
        self.visualizer.add_packet(src_ip, dst_ip, protocol, size)
        self.log_ip_address(src_ip)
        self.log_ip_address(dst_ip)
    ...
    def log_ip_address(self, ip):
        try:
            domain = socket.gethostbyaddr(ip)[0]
        except socket.herror:
            domain = "No domain"

        log_entry = f"{ip} - {domain}\n"

        log_directory = os.getcwd()
        log_file_path = os.path.join(log_directory, "WinEtherApeLog.txt")

        try:
            with open(log_file_path, "a") as log_file:
                log_file.write(log_entry)
                log_file.flush()  # Ensure the data is written to the file immediately
            self.flash_log_indicator()
        except Exception as e:
            print(f"Failed to write to log file: {e}")

    def flash_log_indicator(self):
        self.log_light.setVisible(True)
        self.log_text.setVisible(True)
        QTimer.singleShot(200, self.hide_log_indicator)  # Flash for 200 milliseconds

    def hide_log_indicator(self):
        self.log_light.setVisible(False)
        self.log_text.setVisible(False)
    ...
    def update_log_indicator_position(self):
        self.log_light.setPos(self.width() - 100, 10)  # Adjusted for better visibility
        self.log_text.setPos(self.width() - 80, 10)   # Adjusted for better visibility
